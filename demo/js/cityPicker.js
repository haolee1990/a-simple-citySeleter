...
;(function($){
	...
	CityPicker.prototype.checkDefault=function(cityData,option){
		...
		if(option.prov&&option.city){
			try{
				...
				cityInfo=this.searchCityData(provInfo.city,option.city);
	
	//			distInfo=searchCityData(cityInfo.children,option.dist);
	
				for(key in provInfo.city){
					cityList+='<div class="picker-item">'+provInfo.city[key].name+'</div>';
				}
				for(key in cityInfo.area){
					distList+='<div class="picker-item">'+cityInfo.area[key]+'</div>';
				}
	...
	CityPicker.prototype.init=function(data){
		...
		for(key in cityData){
			provList+='<div class="picker-item">'+cityData[key].name+'</div>';
		}
		for(key in cityData[0].city){
			cityList+='<div class="picker-item">'+cityData[0].city[key].name+'</div>';
		}
		for(key in cityData[0].city[0].area){
			distList+='<div class="picker-item">'+cityData[0].city[0].area[key]+'</div>';
		}
			...
			provGroup.myPicker({
				...
				callback:function(obj,index){
					...
					for(key in city.city){
						cityList+='<div class="picker-item">'+city.city[key].name+'</div>';
					}
					for(key in city.city[0].area){
						distList+='<div class="picker-item">'+city.city[0].area[key]+'</div>';
					}
					cityGroup.html(cityList).css({
						"-webkit-transform":"translate3d(0px,80px,0px)",
						"-webkit-transition-duration":"0s"
			...
			cityGroup.myPicker({
				...
				callback:function(obj,index){
				    ...
				    if(!dist){
				    	...
				    	var cityObj=_this.searchCityData(provObj.city,city);
						dist=cityObj;
				    }
					distList='';
					for(key in dist.area){
						distList+='<div class="picker-item">'+dist.area[key]+'</div>';
					}
					distGroup.html(distList).css({
						"-webkit-transform":"translate3d(0px,80px,0px)",
						"-webkit-transition-duration":"0s"
	...
	$.fn.cityPicker=function(options){
		...
		this.each(function(){
			...
			$.getJSON('AreaData.txt',function(data){
				citypicker.init(data);
			});	
		});
	}
